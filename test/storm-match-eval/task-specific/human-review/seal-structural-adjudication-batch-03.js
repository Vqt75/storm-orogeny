...
import { DEFAULT_STRUCTURAL_ADJUDICATION_BATCH_03_LOG_PATH } from './record-structural-adjudication-batch-03.js';
import { DEFAULT_STRUCTURAL_COMPARISON_MATRIX_PATH } from './seal-structural-adjudication-batch-01.js';

export const STRUCTURAL_ADJUDICATION_BATCH_03_SEALED_AT = '2026-09-08T11:45:13.945Z';
export const DEFAULT_STRUCTURAL_ADJUDICATION_BATCH_03_SEAL_PATH = structuralAdjudicationSealPathForJournal(
...
export function buildStructuralAdjudicationBatch03Seal({
  ...
  const seal = createStructuralAdjudicationSeal({
    ...
    sourceMatrix,
    journalPath,
    sealedAt: STRUCTURAL_ADJUDICATION_BATCH_03_SEALED_AT
  });
  const validation = validateStructuralAdjudicationSeal(seal, { log, sourceMatrix, journalPath });
  if (!validation.ok) throw new Error(`Generated Batch 03 seal is invalid: ${canonicalJson(validation.errors)}`);
  if (seal.eventCount !== 10 || seal.provenance !== HUMAN_ADJUDICATION_PROVENANCE) {
    throw new Error('Batch 03 seal must bind exactly ten HUMAN_ADJUDICATION events');
  }
  return seal;
}

export function materialiseStructuralAdjudicationBatch03Seal({
  journalPath = DEFAULT_STRUCTURAL_ADJUDICATION_BATCH_03_LOG_PATH,
  sourceMatrixPath = DEFAULT_STRUCTURAL_COMPARISON_MATRIX_PATH,
  checkOnly = false
} = {}) {
  const expected = buildStructuralAdjudicationBatch03Seal({ journalPath, sourceMatrixPath });
  ...
  if (existsSync(sealPath)) {
    ...
    if (canonicalJson(actual) !== canonicalJson(expected)) {
      throw new Error(`Existing Batch 03 seal differs from expected immutable seal: ${sealPath}`);
    }
  } else if (checkOnly) {
    throw new Error(`Missing Batch 03 seal: ${sealPath}`);
  } else {
    persistStructuralAdjudicationSeal({
      journalPath,
      sourceMatrixPath,
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  const result = materialiseStructuralAdjudicationBatch03Seal({ checkOnly: process.argv.includes('--check') });
